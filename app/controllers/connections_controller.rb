class ConnectionsController < ApplicationController
  before_action :set_customer, only: %i[index new]
  ...
  def create
    ...
    if connection.present?
      redirect_to new_customer_connection_path,
                  alert: "Connection with Id = '#{connection.connection_string_id}' already exists"
      return
    end

    customer = Customer.find(connection_params[:customer_id])
    response = ApiHelper.create_connection(customer.customer_string_id, connection_params[:country_code],
                                           connection_params[:provider_code])
