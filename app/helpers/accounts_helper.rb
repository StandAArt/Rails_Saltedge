module AccountsHelper
  def self.create_update_accounts_for_connection(connection_string_id, api_call_number, from_id = nil)
    ...
    connection = Connection.where(connection_string_id:).first_or_initialize
    ...
    if !response['error'].present? && connection.present?
      ...
      Account.where(connection_id: connection.id).each do |account|
        TransactionsHelper.create_update_transactions_for_accounts(connection_string_id,
                                                                   account.account_string_id, account.id, 4)
      end
    end
